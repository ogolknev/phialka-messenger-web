...
const error = ref('')
const channelPhotoSrc = computed(() => channelEditFormStore.form.photo ? URL.createObjectURL(channelEditFormStore.form.photo) : undefined)
...
onMounted(async () => {
  ...
  channelEditFormStore.form.photo = channel?.photo?.url ? await (await fetch(channel.photo.url)).blob() : null
})
</script>
...
<template>
  <div class="channel-edit-widget">
    <form class="channel-edit-form" @submit.prevent="submitChannelEdit">
      ...
      <TextInput v-model="channelEditFormStore.form.name" class="name-input" placeholder="Name"></TextInput>
      <TextArea v-model="channelEditFormStore.form.description" class="description-input"
